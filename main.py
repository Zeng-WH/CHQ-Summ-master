...
def main():
    ...
    parser.add_argument("--batch_size", type=int, default=1, help="Training batch size for one process.", )
    ...
    parser.add_argument("--dataset_dir", type=str, help="Directory for dataset.", default='dataset/meqsum_split')
    ...
    if args.mode == 'train':
        ...
        DATA_VAL = os.path.join(args.dataset_dir, 'dev.json')

        if args.train_mode == 'mle':
            # tokenzier for encoding the text
            tokenizer = ProphetNetTokenizer.from_pretrained("/home/disk1/dgt2021/zwh/model/prophetnet-large-uncased")
            model = ProphetNetForConditionalGeneration.from_pretrained("/home/disk1/dgt2021/zwh/model/prophetnet-large-uncased")
        ...
        if args.train_mode == 'mle':
            ...
            for epoch in range(args.num_epochs):
                ...
                if r_l > best_r_l:
                    ...
                    output_model_file = os.path.join(model_file_name, WEIGHTS_NAME)
                    output_config_file = os.path.join(model_file_name, CONFIG_NAME)
                    torch.save(model_to_save.state_dict(), output_model_file)
                    model_to_save.config.to_json_file(output_config_file)
                    tokenizer.save_vocabulary(model_file_name)
